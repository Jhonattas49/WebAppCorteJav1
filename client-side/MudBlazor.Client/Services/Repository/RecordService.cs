...
namespace MudBlazor.Client.Services.Repository
{
    public class RecordService
    {
        ...
        public RecordService(AuthService authService, HttpClient httpClient)
        {
        ...
        public async Task<ApplicationUser> GetRegisterByEmailAsync(string email)
        {
        ...
        public async Task<ResponseData<bool>> DeleteUserAsync(string email)
        {
            await Task.Delay(100);
            return new ResponseData<bool>
            {
                Success = true,
                Message = "Este item foi cancelado"
            };
        }
        ...
        public async Task<ResponseData<InputLoginRequest>> PostRecordAsync(ApplicationUser appuser)
        {

            try
            {
                appuser.Roles = new List<string> { "66cfab740de76f6a2da3384d" };
                // Convertendo o objeto role para JSON
                var roleJson = new StringContent(JsonSerializer.Serialize(appuser), Encoding.UTF8, "application/json");

                // Fazendo a requisição PUT para a API Node.js
                var response = await _httpClient.PostAsync("/Record", roleJson);

                // Verificando se a requisição foi bem-sucedida
                if (response.IsSuccessStatusCode)
                {
                    // Lendo o conteúdo da resposta como JSON
                    var rolesJson = await response.Content.ReadFromJsonAsync<ResponseData<InputLoginRequest>>();


                    return new ResponseData<InputLoginRequest>
                    {
                        Success = rolesJson!.Success,
                        Message = rolesJson!.Message,
                        Data = rolesJson!.Data
                    };
                }
                ...
                return new ResponseData<InputLoginRequest>
                {
                    Data = default,
                    Success = false,
                    Message = "Falha ao criar o registro"
                };
            }
            catch (Exception ex)
            {
